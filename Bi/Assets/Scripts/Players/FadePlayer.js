// @input float triggerTime = 5.0 {"label": "Trigger Time (seconds)"}
// @input SceneObject targetObject {"label": "Target Object"}
// @input string fadeType = "fadeIn" {"widget": "combobox", "values": [{"label": "Fade In", "value": "fadeIn"}, {"label": "Fade Out", "value": "fadeOut"}, {"label": "Fade To", "value": "fadeTo"}]}
// @input float targetOpacity = 0.5 {"label": "Target Opacity", "showIf": "fadeType", "showIfValue": "fadeTo"}
// @input float fadeDuration = 1.0 {"label": "Fade Duration"}
// @input string tweenEasing = "easeOutCubic" {"widget": "combobox", "values": [{"label": "Linear", "value": "linear"}, {"label": "Ease Out Cubic", "value": "easeOutCubic"}, {"label": "Ease In Cubic", "value": "easeInCubic"}, {"label": "Ease In Out Cubic", "value": "easeInOutCubic"}]}
// @input bool enableOnStart = false {"label": "Enable Object On Start"}

var fadeState = {
    isRunning: false,
    isPaused: false,
    startTime: 0,
    pausedTime: 0,
    updateEvent: null,
    material: null,
    startAlpha: 0,
    targetAlpha: 0
};

function onTrigger() {
    if (!script.targetObject) return;
    
    print("FadePlayer: Triggered at " + script.triggerTime);
    
    if (script.enableOnStart) {
        script.targetObject.enabled = true;
    }
    
    var visual = script.targetObject.getComponent("Component.RenderMeshVisual");
    if (!visual) {
        print("FadePlayer: No RenderMeshVisual found");
        return;
    }
    
    var material = visual.getMaterial(0);
    if (!material) {
        print("FadePlayer: No material found");
        return;
    }
    
    fadeState.material = material;
    fadeState.startAlpha = material.getPass(0).baseColor.a;
    
    switch(script.fadeType) {
        case "fadeIn":
            fadeState.targetAlpha = 1.0;
            break;
        case "fadeOut":
            fadeState.targetAlpha = 0.0;
            break;
        case "fadeTo":
            fadeState.targetAlpha = script.targetOpacity;
            break;
        default:
            fadeState.targetAlpha = 1.0;
    }
    
    startFade();
}

function startFade() {
    fadeState.isRunning = true;
    fadeState.isPaused = false;
    fadeState.startTime = getTime();
    fadeState.pausedTime = 0;
    
    fadeState.updateEvent = script.createEvent("UpdateEvent");
    fadeState.updateEvent.bind(updateFade);
}

function updateFade() {
    if (!fadeState.isRunning || fadeState.isPaused) return;
    
    var currentTime = getTime();
    var elapsed = (currentTime - fadeState.startTime) - fadeState.pausedTime;
    var progress = Math.min(elapsed / script.fadeDuration, 1.0);
    var easedProgress = getEasingValue(progress);
    
    var currentAlpha = fadeState.startAlpha + (fadeState.targetAlpha - fadeState.startAlpha) * easedProgress;
    var color = fadeState.material.getPass(0).baseColor;
    fadeState.material.getPass(0).baseColor = new vec4(color.r, color.g, color.b, currentAlpha);
    
    if (progress >= 1.0) {
        fadeState.isRunning = false;
        script.removeEvent(fadeState.updateEvent);
        if (fadeState.targetAlpha <= 0) {
            script.targetObject.enabled = false;
        }
        print("FadePlayer: Animation complete");
    }
}

function getEasingValue(progress) {
    switch(script.tweenEasing) {
        case "linear":
            return progress;
        case "easeInCubic":
            return progress * progress * progress;
        case "easeOutCubic":
            return 1 - Math.pow(1 - progress, 3);
        case "easeInOutCubic":
            return progress < 0.5 
                ? 4 * progress * progress * progress 
                : 1 - Math.pow(-2 * progress + 2, 3) / 2;
        default:
            return 1 - Math.pow(1 - progress, 3);
    }
}

function onPaused() {
    if (fadeState.isRunning) {
        fadeState.isPaused = true;
        fadeState.pauseStartTime = getTime();
        print("FadePlayer: Paused fade animation");
    }
}

function onResumed() {
    if (fadeState.isRunning && fadeState.isPaused) {
        fadeState.isPaused = false;
        fadeState.pausedTime += getTime() - fadeState.pauseStartTime;
        print("FadePlayer: Resumed fade animation");
    }
}

if (global.sequencer) {
    global.sequencer.api.subscribe(script.triggerTime, onTrigger);
    global.sequencer.api.onPaused(onPaused);
    global.sequencer.api.onResumed(onResumed);
    print("FadePlayer: Subscribed to sequencer at time " + script.triggerTime);
} else {
    print("FadePlayer: Sequencer not found!");
}