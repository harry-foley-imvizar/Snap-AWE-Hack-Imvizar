...
function onTrigger() {
    ...
    if (!visual.getMaterial(0)) {
        var newMaterial = visual.createMaterial("Unlit");
        visual.setMaterial(0, newMaterial);
    ...
    // Set the initial color if baseColor doesn't exist
    if (!material.getPass(0).baseColor) {
        material.getPass(0).baseColor = new vec4(1, 1, 1, 1);
    }
    
    startAlpha = material.getPass(0).baseColor.a;
    ...
    if (script.fadeType === "fadeIn") {
        material.getPass(0).baseColor = new vec4(1, 1, 1, startAlpha);
    }
    ...
    updateEvent.bind(function() {
        ...
        material.getPass(0).baseColor = new vec4(1, 1, 1, currentAlpha);
